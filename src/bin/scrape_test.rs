...
//     c.init_logger()?;
//
//     // use an ID from existing DB just for testing
//     let site_id = Uuid::parse_str("7c6144a8-ce34-4e4c-96a7-f48dbcef25ae")?;
//     let mut r = Restaurant::new_for_site("Testaurant", site_id);
//     r.add(Dish::new("Pasta").for_restaurant(r.restaurant_id));
//     r.add(Dish::new("Gröt").for_restaurant(r.restaurant_id));
//     let pg = c.get_pg_pool().await?;
//     db::update_restaurant(&pg, r).await?;
//
//     Ok(())
// }
